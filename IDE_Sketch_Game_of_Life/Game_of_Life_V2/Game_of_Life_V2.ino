...
int briansBrainSizeSelection = 1;   // 1=Small, 2=Medium, 3=Large, 4=Random, 5=Custom
bool briansBrainWasCustom = false;  
uint32_t briansBrain_hash = 0;          // Hash for pattern detection
uint32_t briansBrain_prev_hash = 0;     // Previous hash for comparison
uint32_t briansBrain_short_hash = 0;    // Short interval hash
int briansBrain_lowCellCounter = 0;     // Count consecutive generations with few cells
...
break;
 ...
 // Row 1: 5 empty, 3 cells, 10 empty, 3 cells, 5 empty
    ...
    cellCount = 0;
    uint32_t bb_hash = 5381;  // Hash for pattern detection
    max_cellCount = 0;
    generation = 0;
}
...
void updateBriansBrain() {
    ...
    cellCount = 0;
    uint32_t bb_hash = 5381;  // Declare BEFORE the loop, not inside
    ...
    for (int x = 0; x < BOARD_WIDTH; x++) {
        ...
        for (int y = 0; y < BOARD_HEIGHT; y++) {
            ...
            // Update hash for pattern detection
            bb_hash = ((bb_hash << 5) + bb_hash) ^ ((new_board[x] >> y) & 1);
            bb_hash = ((bb_hash << 5) + bb_hash) ^ ((new_dying[x] >> y) & 1);
        }
    }
...
// Update the global arrays
    ...
    // Store hash for pattern detection
    briansBrain_hash = bb_hash;
}    
...
void handleDirectionalButtons() {
    ...
    if (gameMode == 4) {  // Symmetric Game
    ...
    } else if (gameMode == 8 && briansBrainMode) {  // Brian's Brain with custom support
        if (briansBrainWasCustom) {
            // Was started as custom - clear board and return to edit mode
            clearBoard();
            editMode = true;
            ...
            // Was started as preset/random - regenerate pattern
            clearBoard();
            ...
            generation = 0;
            cellCount = 0;
            frame_start_t = millis() - frameDelays[currentSpeedIndex];
        }
        ...
        if (seedsWasCustom) {
            ...
            // Was started as random - generate new random pattern
            initializeSeeds();
        }
    } else {
        if (gameMode == 5) {  // Day & Night mode needs sparse initialization
            ...
            randomizeBoard();  // Normal density for Conway's Life, etc.
        }
    }
    
    if (!seedsMode || !seedsWasCustom) {  // Don't reset these for custom Seeds/Brian's returning to edit
        if (!(gameMode == 8 && briansBrainWasCustom)) {  // Also skip for custom Brian's Brain
            generation = 0;
            start_t = micros();
    ...
    // RIGHT button does the same thing as LEFT for pattern regeneration
    if (gameMode == 4) {  // Symmetric Game
        currentSymmetryType = rand() % 3 + 1;
    ...
    if (!seedsMode || !seedsWasCustom) {
        if (!(gameMode == 8 && briansBrainWasCustom)) {
            generation = 0;
            start_t = micros();
            frame_start_t = millis();
        }
...
void handleSetButton() {
    ...
    if (currentlyPressed && setButtonPressed && editMode && !inMenuMode) {
        ...
        if (pressDuration >= 800) {  // Long press detected - start immediately
            ...
            // For Seeds mode, ensure seedsMode flag is set
            if (gameMode == 6) {
                seedsMode = true;
            }
    ...
    cellCount = 0;
    generation = 0;
    frame_start_t = now - frameDelays[currentSpeedIndex];
    start_t = micros();
    buttonProcessingInProgress = false;
...
void handleButtonB() {
    ...
    if (buttonBPressed && !buttonBWasPressed) {
        ...
        // If currently in an Alt Game (playing), return to Alt Games menu
        if (briansBrainMode || dayNightMode || langtonAntMode || seedsMode) {
            // Clean up current game state
            briansBrainMode = false;
            dayNightMode = false;
            langtonAntMode = false;
            seedsMode = false;
            editMode = false;
            showingStats = false;
            briansBrain_lowCellCounter = 0;
            
            // Go to Alt Games menu
            inMenuMode = true;
            inAltGamesSubMenu = true;
            altGameSelection = 1;
            showMenu();
...
static inline uint32_t rotate_shift_three(uint32_t x, int n) {
    ...
    if (!editMode) {
        ...
        if (showingStats) {
            // Stay in statistics display for 2 seconds
            if (current_time - statsStartTime >= 2000) {
                showingStats = false;
                display.clearDisplay();
                display.display();
                
                // Return to appropriate menu based on game mode
                if (gameMode == 8 && briansBrainMode) {
                    // Return to Brian's Brain submenu
                    briansBrainMode = false;
                    inMenuMode = true;
                    inBriansBrainSubMenu = true;
                    inAltGamesSubMenu = false;
                    ...
                    // Return to main menu for other games
                    inMenuMode = true;
                    menuSelection = 1;
                }
                showMenu();
    ...
    break;
            ...
            case 8: // Brian's Brain
                    if (briansBrainMode) {
                        updateBriansBrain();
                        drawBriansBrain();
                        
                        // Track low cell counts
                        if (cellCount < 10 && generation > 20) {
                            briansBrain_lowCellCounter++;
                            
                            // If cells stay low for 15 consecutive generations, game over
                            if (briansBrain_lowCellCounter >= 15) {
                                startShowingStats();
                                briansBrain_lowCellCounter = 0;  // Reset for next game
                                return;
                            }
                        } else {
                            // Reset counter if cell count goes back up
                            briansBrain_lowCellCounter = 0;
                        }
                        
                        // Also end if completely dead
      ...
      else if (editMode && (gameMode == 2 || gameMode == 6 || (gameMode == 8 && briansBrainWasCustom))) {  // Show cursor for custom games, Seeds, and Brian's Brain
