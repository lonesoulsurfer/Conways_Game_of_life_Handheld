...
void handleDirectionalButtons() {
    ...
    briansBrainSizeSelection = (briansBrainSizeSelection > 1) ? briansBrainSizeSelection - 1 : 4;
        } else if (inSeedsSubMenu) {
    ...
    briansBrainSizeSelection = (briansBrainSizeSelection < 4) ? briansBrainSizeSelection + 1 : 1;  
            } else if (inSeedsSubMenu) {
...
} else if (seedsMode) {  // Seeds mode reset - FIXED VERSION
        if (seedsWasCustom) {
    ...
    if (!seedsMode || !seedsWasCustom) {  // Don't reset these for custom Seeds returning to edit
        generation = 0;
        start_t = micros();
        frame_start_t = millis();
    }
    ...
    } else if (gameMode == 8 && briansBrainMode) {  // Brian's Brain
        // Clear both arrays and regenerate pattern based on stored size
        clearBoard(); // This clears both board[] and briansBrain_dying[]
...
static inline uint32_t rotate_shift_three(uint32_t x, int n) {
    ...
    break;
      ...
      else if (editMode && (gameMode == 2 || gameMode == 6)) {  // Show cursor for custom games and custom Seeds
