...
void handleDirectionalButtons() {
    ...
    // Increase minimum time between button checks
    if (now - lastProcessTime < 100) return;  // Changed from 50 to 100ms
    if (now - last_button_check_time < 50) return;
    
    last_button_check_time = now;
    int btnValue = readButtonValue();
   
   // Shorter debounce for cursor movement in edit mode
    ...
    if (editMode) {
        if (isBtnUp(btnValue)) {
            cursorY = (cursorY - 1 + BOARD_HEIGHT) % BOARD_HEIGHT;
            lastDirectionalPress = now;
            flashLED();
        }
        else if (isBtnDown(btnValue)) {
            cursorY = (cursorY + 1) % BOARD_HEIGHT;
            lastDirectionalPress = now;
            flashLED();
        }
        else if (isBtnLeft(btnValue)) {
            cursorX = (cursorX - 1 + BOARD_WIDTH) % BOARD_WIDTH;
            lastDirectionalPress = now;
            flashLED();
        }
        else if (isBtnRight(btnValue)) {
            cursorX = (cursorX + 1) % BOARD_WIDTH;
            lastDirectionalPress = now;
            flashLED();
        }
        return;
...
void handleSetButton() {
    ...
    if (now - lastSetPress < 200) return;
    ...
    if (setPressed && !setWasPressed) {
        ...
        if (inMenuMode) {
            ...
            } else if (!justEnteredSubmenu && (now - submenuEntryTime > 300)) {
    ...
    // Check for long press while button is held down
        ...
        if (editMode && gameMode == 2 && pressDuration >= 1000) {
        ...
        else if (!editMode && gameMode == 3 && pressDuration >= 1000) {
            generation = 0;
            start_t = micros();
            frame_start_t = millis();
            currentSpeedIndex = 0;
       ...
       if (justEnteredSubmenu && (now - submenuEntryTime > 100)) {
        ...
        if (editMode && gameMode == 2 && pressDuration < 1000 && !longPressTriggered) {
            toggleCellAtCursor();
            flashLED();
        }
    ...
    if (!editMode) {
        ...
        if (showingStats) {
          ...
          generation++; // Count generations only when simulation actually advances
            ...
            if (cellCount < 5 && generation > 20) {  // Increased from 5 to 20 generations
                // Game is dead (too few cells)
                
                // Trigger statistics display
                if (!showingStats) {  // Remove gameMode check to show stats for both modes
                    startShowingStats();
                }
            ...
            else if (short_check_hash == game_hash && generation > 30) {  // Added minimum generation check
                // Pattern repeating
      ...
      display.display();
        ...
        if (cursorX < 0 || cursorX >= BOARD_WIDTH) cursorX = BOARD_WIDTH / 2;
        if (cursorY < 0 || cursorY >= BOARD_HEIGHT) cursorY = BOARD_HEIGHT / 2;
        display.clearDisplay();
        for (int x = 0; x < BOARD_WIDTH; x++) {
            uint32_t col = board[x];
            // Skip offscreen columns for better performance
